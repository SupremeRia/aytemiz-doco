...
import { completeTaskAction,editEvidenceAction,reviewTaskAction,startTaskAction } from "@/app/station/[stationSlug]/tasks/actions";
...
export default async function TaskDetailPage({params,searchParams}:{params:Promise<{stationSlug:string;taskId:string}>;searchParams:Promise<{error?:string}>}){
 ...
 return <main className="shell app-shell">
  ...
  {detail.evidence.length?<section className="dashboard-section"><SectionHeader title="Tamamlama kayıtları"/><div className="content-stack">{detail.evidence.map(e=><Card key={e.id}><div className="card-row"><strong>{e.submitter_name}</strong>{e.edited_at?<Badge>Düzenlendi</Badge>:null}</div><p>{e.description??"Açıklama eklenmedi."}</p><time className="muted">Yapıldığı saat: {format(e.completed_at)}</time>{e.can_edit?<details><summary className="ui-button ui-button--secondary">Kanıtı düzenle</summary><form action={editEvidenceAction} className="form-stack mt-4"><input type="hidden" name="taskId" value={taskId}/><input type="hidden" name="evidenceId" value={e.id}/><input type="hidden" name="stationSlug" value={stationSlug}/><label><span>Yapılan işlem</span><Textarea name="description" defaultValue={e.description??""}/></label><label><span>Yapıldığı saat</span><Input name="completedAt" type="datetime-local" defaultValue={e.completed_at.slice(0,16)}/></label><Button>Kanıtı güncelle</Button></form></details>:null}</Card>)}</div></section>:null}
