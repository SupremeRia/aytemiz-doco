...
import { requirePermission } from "@/lib/auth/server-authorization";
import { RolePermissionMatrix,UserPermissionEditor } from "@/components/access-management";
...
const sectionDetails={
  ...
  "role-matrix":["Rol–Yetki Matrisi","Rollerin atandığı bölge ve istasyonlarda kullanacağı varsayılan izinleri yönetin."],
  "user-permissions":["Kullanıcı Yetki İstisnaları","Rol izinlerinin üzerine çıkan kullanıcıya özel izin veya ret kararı ekleyin."],
...
const sectionPermission:Record<string,string>={"pending-users":"view_all_users",users:"view_all_users",stations:"view_all_stations",roles:"create_role",permissions:"create_permission","role-matrix":"roles.assign_permissions","user-permissions":"roles.assign_permissions",operators:"manage_operators","audit-logs":"view_audit_logs"};
...
export default async function AdminSection({params,searchParams}:{params:Promise<{section:string}>;searchParams:Promise<{saved?:string;error?:string}>}){
  ...
  await requirePermission(sectionPermission[section]);
  if(section==="role-matrix"||section==="user-permissions")return <main className="shell"><Link href="/admin" className="back-link"><ArrowLeft size={17}/>Yönetici paneline dön</Link><header className="mt-9"><p className="eyebrow">Erişim yönetimi</p><h1 className="mt-2 text-3xl font-bold">{title}</h1><p className="muted mt-3">{description}</p></header>{notice.saved?<div role="status" className="mt-6 rounded-2xl border border-emerald-900 bg-emerald-950/30 p-4 text-emerald-200">İşlem başarıyla tamamlandı.</div>:null}{notice.error?<div role="alert" className="mt-6 rounded-2xl border border-red-900 bg-red-950/30 p-4 text-red-200">{notice.error}</div>:null}{section==="role-matrix"?<RolePermissionMatrix/>:<UserPermissionEditor/>}</main>;
