...
import { Activity, Building2, CheckSquare, ChevronRight, FileText, KeyRound, MessageSquare, Newspaper, Shield, UserCheck, Users } from "lucide-react";
...
export default async function Admin() {
  ...
  const contentEntries=[permissions.has("messages.moderate")?["/dashboard#stations",MessageSquare,"Sohbet moderasyonu","İstasyon sohbetlerine çalışma alanından erişin"]:null,permissions.has("files.upload")?["/dashboard#stations",FileText,"Dosyalar","İstasyon belgelerini çalışma alanından yönetin"]:null,permissions.has("news.create")?["/news/manage",Newspaper,"Haberler","Kurumsal haber oluşturun ve yayınlayın"]:null,permissions.has("checklists.create_template")?["/admin/checklist-templates",CheckSquare,"Checklist Şablonları","Günlük ve vardiya iş akışlarını tanımlayın"]:null].filter(Boolean) as [string,typeof MessageSquare,string,string][];
  return <main className="shell"><header className="flex items-center justify-between"><Brand/><span className="rounded-full border border-red-900 bg-red-950/50 px-3 py-1 text-xs font-bold text-red-300">YÖNETİM</span></header><section className="mt-12"><p className="eyebrow">Kontrol merkezi</p><h1 className="mt-2 text-3xl font-bold">Yönetici paneli</h1><p className="muted mt-2">Yalnızca yetkili olduğunuz yönetim bölümleri gösterilir.</p></section><div className="mt-8 grid gap-3 md:grid-cols-2">{visible.map(([slug,Icon,title,text])=><Link key={slug} href={`/admin/${slug}`} className="glass flex items-center gap-4 rounded-2xl p-4 text-left transition hover:-translate-y-0.5 hover:border-zinc-600"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-zinc-800 text-rose-400"><Icon/></span><span className="min-w-0 flex-1"><strong className="block">{title}</strong><small className="muted block truncate">{text}</small></span>{slug in countIndex?<span className="hidden rounded-full bg-zinc-800 px-2 py-1 text-[10px] text-zinc-300 sm:block">{counts[countIndex[slug]]} kayıt</span>:null}<ChevronRight size={18} className="text-zinc-600"/></Link>)}{contentEntries.map(([href,Icon,title,text])=><Link key={title} href={href} className="glass flex items-center gap-4 rounded-2xl p-4"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-zinc-800 text-rose-400"><Icon/></span><span className="flex-1"><strong className="block">{title}</strong><small className="muted">{text}</small></span><ChevronRight size={18}/></Link>)}</div><div className="mt-8 rounded-2xl border border-amber-900/50 bg-amber-950/20 p-4 text-sm text-amber-100"><strong>Kritik işlem koruması:</strong> Yetki ve OP değişiklikleri ayrıca sunucu ve RLS katmanında doğrulanır.</div><AppNav/></main>;
}
