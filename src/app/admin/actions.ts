...
export async function replaceRolePermissions(form:FormData){const roleId=value(form,"roleId",36);if(!UUID.test(roleId))fail("role-matrix","Geçersiz rol.");const permissionIds=form.getAll("permissionIds").map(String).filter(id=>UUID.test(id));const{supabase}=await authorizedClient("roles.assign_permissions");const{error}=await supabase.rpc("replace_role_permissions",{target_role:roleId,permission_ids:permissionIds});if(error)fail("role-matrix",userError(error));done("role-matrix")}
export async function setUserPermission(form:FormData){const userId=value(form,"userId",36),permissionId=value(form,"permissionId",36),scope=value(form,"scope",16),decision=value(form,"decision",8);if(!UUID.test(userId)||!UUID.test(permissionId)||!["global","assigned","region","station"].includes(scope)||!["allow","deny"].includes(decision))fail("user-permissions","Yetki istisnası alanlarını kontrol edin.");const{supabase}=await authorizedClient("roles.assign_permissions");const{error}=await supabase.rpc("set_user_permission",{target_user:userId,target_permission:permissionId,grant_value:decision==="allow",target_scope:scope,target_station:optionalUuid(form,"stationId"),target_region:optionalUuid(form,"regionId")});if(error)fail("user-permissions",userError(error));done("user-permissions")}
export async function removeUserPermission(form:FormData){const exceptionId=value(form,"exceptionId",36);if(!UUID.test(exceptionId))fail("user-permissions","Geçersiz istisna.");const{supabase}=await authorizedClient("roles.assign_permissions");const{error}=await supabase.rpc("remove_user_permission",{target_exception:exceptionId});if(error)fail("user-permissions",userError(error));done("user-permissions")}
